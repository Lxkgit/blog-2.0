...
let { size, total, sensorDataList, selectSensorDataPageFun, selectSensorDataByIdFun } = sensorFun();
...
function sensorFun() {
  ...
  const selectSensorDataByIdFun = (id: any, page: any) => {
    selectSensorDataListApi({ "pageNum": page, "pageSize": size.value, "sensorId": id }).then((res: any) => {
      if (res.code === 200) {
  ...
  return { size, total, sensorDataList, selectSensorDataPageFun, selectSensorDataByIdFun }
}
