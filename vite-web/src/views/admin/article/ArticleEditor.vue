    ...
    <el-dialog v-model="saveDialog" title="文章信息" width="40%">
      <el-form class="demo-form-inline">
        <el-form-item label="文章标题" style="width: 80%">
          <el-input v-model="article.data.title" placeholder="文章标题" clearable />
        </el-form-item>
        <el-form-item label="文章描述" style="width: 80%">
          <el-input v-model="article.data.contentMemo" maxlength="300" placeholder="请输入文章描述" show-word-limit
            type="textarea" />
        </el-form-item>
        <el-form-item label="文章封面">
          <ImgUpload @upload="imgUpload"
            :imgList="(article.data.contentImg === null || article.data.contentImg === undefined || article.data.contentImg === '') ? [] : [article.data.contentImg]"
        ...
        <el-form-item label="文章标签">
          <div style="display: flex; justify-content: space-between; align-items: center; ">
            <el-popover placement="top" title="文章标签" trigger="click" :width="360">
              <el-input style="padding-bottom: 10px;" class="w-50 m-2" placeholder="搜索文章标签" />
              <el-row :gutter="24" style="height: 200px;">
                <el-col :span="6" style="height: 100%; overflow:auto;" class="label_type_col">
                  <ul v-for="(labelType, index) in labelTypeList" style="list-style-type: none; padding: 0; margin: 0;">
                    <li style="cursor: pointer; margin-bottom: 5px; " @click="selectLabelList(labelType, index)"
                      class="article_label_tag" :class="labelTypeActive == index ? 'article_label_tag_active' : ''"
                      :key="index">
                      {{ labelType.label }}
                    </li>
                  </ul>
                ...
                <el-col :span="18" style="">
                  ...
                  <el-input v-if="tagInputVisible" ref="InputRef" v-model="inputTagValue" class="ml-1 w-20" size="small"
                    @keyup.enter="tagInputHandleConfirm" @blur="tagInputHandleConfirm" />
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="saveArticleFun">
            保存
          </el-button>
        </span>
      </template>
...
import { saveArticleApi, updateArticleApi, getArticleTypeTreeApi, getArticleLabelTypeListApi, saveArticleLabelApi } from "@/api/content"
...
// 模块方法引入
let { article, type, labels, saveArticleDialogFun, imgUpload, saveArticleFun, updateArticleFun } = articleFun();
let { labelList, labelTypeList, labelTypeActive, tagInputVisible, inputTagValue, showTagInput,
  tagInputHandleConfirm, getArticleLabelTypeListFun, addLabel, deleteLabel, selectLabelList } = articleLabelFun();
let { typeList, selectType, articleType } = articleTypeFun()
...
function articleLabelFun() {
  ...
  const addLabel = (label: any) => {
    ...
    if (labels.value.length + 1 > articleLabelMaxNum.value) {
      ElMessage({ message: '文章标签上限为' + articleLabelMaxNum.value + '个', type: 'error' })
    } else {
      if (!flag) {
