...
public final class RedisUtil {
    ...
    private static class SaveSessionCallback implements SessionCallback {
        private final List<RedisHashBean> redisHashBeans;

        public SaveSessionCallback(List<RedisHashBean> redisHashBeans) {
            this.redisHashBeans = redisHashBeans;
        }

        @Override
        public Object execute(RedisOperations operations) throws DataAccessException {
            for (RedisHashBean redisHashBean : redisHashBeans) {
                operations.opsForHash().put(redisHashBean.getKey(), redisHashBean.getField(),
                                JSON.toJSONString(redisHashBean.getValue()));
            }
            return null;
        }
    }

    private static class QuerySessionCallback implements SessionCallback {
        private final String key;

        private final List<String> fields;

        public QuerySessionCallback(String key, List<String> fields) {
            this.key = key;
            this.fields = fields;
        }
     ...
     * @return 返回值
     */
    public <T> List<T> pipelineToQuery(String key, List<String> fields, Class<T> clazz) {
        List<Object> responseList = redisTemplate.executePipelined(new QuerySessionCallback(key, fields));
        List<T> result = new ArrayList<>(responseList.size());
        for (Object response : responseList) {
            String resp = toString(response);
            if (StringUtils.isEmpty(resp)) {
                continue;
            }
            result.add(JSON.parseObject(resp, clazz));
        }
        return result;
    }
    ...
    /**
     * 管道方式
     * @param redisHashBeans 存储Bean
     */
    public void pipelineToSave(List<RedisHashBean> redisHashBeans) {
        redisTemplate.executePipelined(new SaveSessionCallback(redisHashBeans));
    }

    @Data
    public static class RedisHashBean {
        private String key;
